...
fn create_and_get_roundtrip() {
    let mgr = manager();
    let r = mgr.create(
        CreateReminder::new("Check dev server", "See if it's still running", due_in(3600), "agent/test")
    ).unwrap();
...
fn get_nonexistent_returns_not_found() {
    let mgr = manager();
    assert!(matches!(mgr.get("no-such-id"), Err(agentstategraph_reminders::ReminderError::NotFound(_))));
}

#[test]
fn cancel_moves_to_cancelled() {
    let mgr = manager();
    let r = mgr.create(CreateReminder::new("t", "i", due_in(60), "a")).unwrap();
...
fn cancel_completed_reminder_returns_error() {
    let mgr = manager();
    let r = mgr.create(
        CreateReminder::new("t", "i", past(1), "a")
    ).unwrap();
    ...
    assert!(matches!(result, Err(agentstategraph_reminders::ReminderError::AlreadyTerminal(..))));
}
...
fn remind_me_promotes_pending_past_due() {
    let mgr = manager();
    let r = mgr.create(CreateReminder::new("overdue", "i", past(10), "a")).unwrap();
...
fn remind_me_does_not_promote_future_reminders() {
    let mgr = manager();
    let r = mgr.create(CreateReminder::new("future", "i", due_in(3600), "a")).unwrap();
    mgr.remind_me().unwrap();
    assert_eq!(mgr.get(&r.id).unwrap().status, ReminderStatus::Pending);
}
...
fn remind_me_orders_by_priority_then_due_at() {
    let mgr = manager();
    let _low = mgr.create(
        CreateReminder::new("low", "i", past(5), "a").with_priority(Priority::Low)
    ).unwrap();
    let _crit = mgr.create(
        CreateReminder::new("critical", "i", past(3), "a").with_priority(Priority::Critical)
    ).unwrap();
    let _high = mgr.create(
        CreateReminder::new("high", "i", past(1), "a").with_priority(Priority::High)
    ).unwrap();
...
fn remind_me_includes_awaiting_permission() {
    let mgr = manager();
    let r = mgr.create(
        CreateReminder::new("needs approval", "i", past(1), "a").with_autonomous(false)
    ).unwrap();
    ...
    assert!(fetched.status == ReminderStatus::Due || fetched.status == ReminderStatus::AwaitingPermission);
}
...
fn snooze_sets_snoozed_status_and_until() {
    let mgr = manager();
    let r = mgr.create(CreateReminder::new("t", "i", past(1), "a")).unwrap();
    mgr.remind_me().unwrap();
...
fn snoozed_reminder_wakes_on_remind_me() {
    let mgr = manager();
    let r = mgr.create(CreateReminder::new("t", "i", past(100), "a")).unwrap();
    mgr.remind_me().unwrap();
...
fn approve_awaiting_permission_moves_to_due() {
    ...
    let r = mgr.create(
        CreateReminder::new("t", "i", past(1), "a").with_autonomous(false)
    ).unwrap();
...
fn start_and_complete_marks_completed() {
    ...
    let done = mgr.record_execution(&r.id, success_record("agent/worker")).unwrap();
...
fn record_execution_without_start_also_works() {
    ...
    let r = mgr.create(CreateReminder::new("t", "i", past(1), "a")).unwrap();
    mgr.remind_me().unwrap();
    let done = mgr.record_execution(&r.id, success_record("agent/a")).unwrap();
    assert_eq!(done.status, ReminderStatus::Completed);
    assert_eq!(done.executions.len(), 1);
}
...
fn failed_execution_returns_to_due() {
    ...
    let after = mgr.record_execution(&r.id, ExecutionRecord {
        started_at: Utc::now(),
        completed_at: None,
...
fn execution_records_task_id() {
    ...
    let r = mgr.create(CreateReminder::new("t", "i", past(1), "a")).unwrap();
    mgr.remind_me().unwrap();
    let done = mgr.record_execution(&r.id, ExecutionRecord {
        started_at: Utc::now(),
        completed_at: None,
        agent_id: "agent/a".into(),
        approved_by: None,
...
fn interval_schedule_resets_to_pending_after_success() {
    let mgr = manager();
    let r = mgr.create(
        CreateReminder::new("recurring", "i", past(1), "a")
            .with_schedule(Schedule::Interval { every_seconds: 3600 })
    ).unwrap();
    mgr.remind_me().unwrap();
    let after = mgr.record_execution(&r.id, success_record("agent/a")).unwrap();
...
fn once_schedule_marks_completed_not_rescheduled() {
    let mgr = manager();
    let r = mgr.create(CreateReminder::new("one-time", "i", past(1), "a")).unwrap();
    mgr.remind_me().unwrap();
    let after = mgr.record_execution(&r.id, success_record("agent/a")).unwrap();
...
fn repeating_reminder_accumulates_execution_history() {
    ...
    for _ in 0..3 {
        ...
        mgr.record_execution(&r.id, success_record("agent/a")).unwrap();
    }
...
fn mark_ref_stale_updates_matching_ref() {
    ...
    let branch_ref = updated.refs.iter().find(|rf| rf.id == "feature/web-demo").unwrap();
...
fn stale_ref_label_preserved() {
    ...
    let r = mgr.create(
        CreateReminder::new("t", "i", due_in(60), "a")
            .with_refs(vec![ReminderRef::branch("feature/x", "Feature X — web server cleanup")])
    ).unwrap();
...
fn list_by_tags() {
    ...
    mgr.create(
        CreateReminder::new("other", "i", due_in(60), "a")
            .with_tags(vec!["other".into()])
    ).unwrap();

    let results = mgr.list(&ReminderFilter {
...
fn list_by_ref_id() {
    ...
    mgr.create(CreateReminder::new("no-ref", "i", due_in(60), "a")).unwrap();
...
fn list_by_created_by() {
    let mgr = manager();
    mgr.create(CreateReminder::new("mine", "i", due_in(60), "agent/alice")).unwrap();
    mgr.create(CreateReminder::new("theirs", "i", due_in(60), "agent/bob")).unwrap();
