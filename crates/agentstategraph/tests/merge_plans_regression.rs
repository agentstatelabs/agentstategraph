...
fn merge_preserves_target_added_plans() {
    ...
    r.set("main", "/plans/p1/status", &Object::string("pending"), opts("add p1")).unwrap();

    // Branch source from the current target state.
    r.branch("source", "main").unwrap();

    // Target adds new plans p2, p3 AFTER the branch point.
    r.set("main", "/plans/p2/status", &Object::string("pending"), opts("add p2")).unwrap();
    r.set("main", "/plans/p3/status", &Object::string("pending"), opts("add p3")).unwrap();

    // Source completes the older plan p1.
    r.set("source", "/plans/p1/status", &Object::string("done"), opts("complete p1")).unwrap();

    // Merge source into target.
    r.merge("source", "main", merge_opts("merge source into main")).unwrap();
...
fn repeated_merges_preserve_all_plans() {
    let r = repo();
    r.set("main", "/plans/p1/status", &Object::string("pending"), opts("p1")).unwrap();

    // First round: source completes p1, target adds p2; merge back.
    r.branch("source", "main").unwrap();
    r.set("main", "/plans/p2/status", &Object::string("pending"), opts("p2")).unwrap();
    r.set("source", "/plans/p1/status", &Object::string("done"), opts("done p1")).unwrap();
    ...
    r.set("source", "/plans/p3/status", &Object::string("pending"), opts("p3")).unwrap();
    r.set("main", "/plans/p4/status", &Object::string("pending"), opts("p4")).unwrap();
...
fn merge_checked_blocks_top_level_deletion() {
    ...
    r.set("main", "/plans/p1/status", &Object::string("pending"), opts("p1")).unwrap();
    r.set("main", "/memory/m1", &Object::string("note"), opts("m1")).unwrap();
    r.branch("source", "main").unwrap();
    // Target diverges so the merge is a real three-way, not a fast-forward.
    r.set("main", "/memory/m2", &Object::string("note2"), opts("m2")).unwrap();
    ...
    assert_eq!(r.head("main").unwrap(), before, "target ref must be unchanged");
    // /plans still intact.
    assert_eq!(r.get("main", "/plans/p1/status").unwrap(), Object::string("pending"));

    // With allow_deletions=true the same merge proceeds and drops /plans.
    r.merge_checked("source", "main", merge_opts("allowed"), true).unwrap();
...
fn preview_merge_summarizes_without_committing() {
    ...
    r.set("main", "/plans/p1/status", &Object::string("pending"), opts("p1")).unwrap();
    r.branch("source", "main").unwrap();
    r.set("main", "/memory/m1", &Object::string("note"), opts("m1")).unwrap(); // target adds /memory
    r.set("source", "/plans/p1/status", &Object::string("done"), opts("done")).unwrap();

    let before = r.head("main").unwrap();
    let preview = r.preview_merge("source", "main").unwrap();
    assert_eq!(r.head("main").unwrap(), before, "preview must not move the ref");
    assert!(preview.removed.is_empty(), "nothing should be removed: {:?}", preview.removed);
    assert!(preview.conflicts.is_empty());
    // /plans subtree changes (p1 -> done); /memory is target-only so unchanged by the merge.
    assert!(preview.changed.contains(&"plans".to_string()), "changed={:?}", preview.changed);
}
